...
class NativeEvalVirtualCubeTest extends BatchTestCase {
  ...
  void testIntermixedDimensionGroupings(Context context) {
    ...
    assertQueryReturns(context.getConnectionWithDefaultRole(),
        ...
        + "select non empty crossjoin(crossjoin(gender.gender.members, warehouse.warehouse.[USA].[CA]), [marital status].[marital status].members) on 0, "
        + " measures.vmUS on 1 from [warehouse and sales]",
        "Axis #0:\n"
        + "{}\n"
        + "Axis #1:\n"
        + "{[Gender].[Gender].[F], [Warehouse].[Warehouse].[USA].[CA], [Marital Status].[M]}\n"
        + "{[Gender].[Gender].[F], [Warehouse].[Warehouse].[USA].[CA], [Marital Status].[S]}\n"
        + "{[Gender].[Gender].[M], [Warehouse].[Warehouse].[USA].[CA], [Marital Status].[M]}\n"
        + "{[Gender].[Gender].[M], [Warehouse].[Warehouse].[USA].[CA], [Marital Status].[S]}\n"
        + "Axis #2:\n"
        + "{[Measures].[vmUS]}\n"
        + "Row #0: 65,336\n"
        + "Row #0: 66,222\n"
  ...
  void testCachedShouldNotBeUsed(Context context) {
    ...
    executeQuery(
        "select non empty crossjoin(gender.gender.members, warehouse.warehouse.[USA].[CA]) on 0, "
  ...
  void testShouldUseCache(Context context) {
    ...
    String mdx =
        "with member measures.vm as 'validmeasure(measures.[unit sales])' "
        + "select non empty "
        + "crossjoin(gender.gender.members, warehouse.warehouse.[USA].[CA]) on 0, "
        + "measures.vm on 1 from [warehouse and sales]";
